...
function walk(ctx, checker) {
    ...
    function check(node) {
        if (matchDeclarationOrder || matchDeclarationOrderOnly) {
            ...
            if (type !== undefined) {
                ...
                var typeName_1 = getDeclarationName(type);
                var groups = [{
                        name: getDeclarationName(type),
                        props: members
                    }];
                var knownPropNames = new Map(members
                    .filter(function (m) { return m.name && m.name.kind !== ts.SyntaxKind.ComputedPropertyName; })
                    .map(function (m) { return [m.name.text, typeName_1]; }));
                var contexualType = checker.getContextualType(node);
                if (tsutils_1.isInterfaceType(contexualType)) {
                    var baseTypes = checker.getBaseTypes(contexualType);
                    for (var _i = 0, baseTypes_1 = baseTypes; _i < baseTypes_1.length; _i++) {
                        ...
                        for (var _a = 0, properties_1 = properties; _a < properties_1.length; _a++) {
                            var declarations = properties_1[_a].declarations;
                ...
                if (groups.every(function (g) { return g.props.every(function (m) {
    ...
    function checkMatchesDeclarationOrder(_a, groups, knownPropNames) {
        var properties = _a.properties;
        function check(members) {
            var memberIndex = 0;
            outer: for (var _i = 0, properties_2 = properties; _i < properties_2.length; _i++) {
        ...
        var fail = check(groups.reduceRight(// from base to derived
        function (members, group) { return members.concat(group.props); }, []));
        if (!fail) {
            return;
        }
        ...
        ctx.addFailureAtNode(fail.name, Rule.FAILURE_STRING_USE_DECLARATION_ORDER(fail.propName, knownPropNames.get(fail.propName)));
    }
}
...
function getDeclarationName(t) {
    var parent = t.parent;
    return tsutils_1.isInterfaceDeclaration(t)
        ? t.name.text
...
function getTypeName(t) {
    var symbol = t.symbol || t.aliasSymbol;
    if (!symbol) {
        return undefined;
    }
